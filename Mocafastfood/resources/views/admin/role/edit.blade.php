...
<div class="content-wrapper">
  ...
  @include('partial.admin.content-header', ['name' => 'Vai trò/', 'key'=>'sửa'])
  <!-- /.content-header -->
          ...
          <form action="{{ route('roles.update', ['id'=> $role->id]) }}" method="post">
            <div class="col-md-12">
              ...
              <div class="form-group">
                ...
                <input type="text" name="name" class="form-control" placeholder="nhập tên role" value="{{$role->name}}">
              </div>
                    ...
                    <div class="row">
                      @foreach ($permissionsItem->permissionsChildren as $permissionChildrenItem)
                        <div class="card-body text-primary md-3 col-md-3" >
                          <h5 class="card-title">
                            <label>
                              <input type="checkbox" class="checkbox_children" name="permissionID[]"
                                      {{$permissionsChecked->contains('id',$permissionChildrenItem->id) ? 'checked':''}} value="{{$permissionChildrenItem->id}}">
                            </label>
